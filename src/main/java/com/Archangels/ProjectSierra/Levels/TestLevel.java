...
import com.Archangels.ProjectSierra.Block.BackgroundBlock;
...
public class TestLevel {
	...
	public TestLevel(Handler handler) {
		...
		// Far right column
		handler.addGameElement(new BasicGround(new Location(160, 640)));
		
		handler.addGameElement(new BackgroundBlock(new Location(280, 640), 40, 40));
		...
		for (int i = 540; i < 660; i += 40) {
			handler.addGameElement(new PassableBlock(new Location(i, 200), 40, 15));
		}

		for (int i = 540; i < 660; i += 40) {
			handler.addGameElement(new PassableBlock(new Location(i, 400), 40, 15));
		}
